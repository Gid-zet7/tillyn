      ...
      <div className="">
        ...
        {orders?.length ? (
          <div className="list-inside space-y-2">
            {orders?.map((order) => (
              <div key={order?._id} className="flex flex-col gap-3">
                <div className={`${poppins.className}`}>
                  Order #{order?._id}
                </div>
                <div>
                  <span className={`${poppins.className}`}>Total amount: </span>
                  <span className="font-extrabold textlg">
                    GHS {order?.total_amount}
                  </span>
                </div>
                <div className="flex gap-2 items-center">
                  <h2 className={`flex gap-2 ${poppins.className}`}>
                    <Package />
                    Order Status:
                  </h2>
                  <button
                    className={`${
                      order?.status === "Delivered"
                        ...
                        ? "bg-black text-white"
                        : "bg-yellow-300"
                    } px-2 py-1 rounded-md flex gap-3`}
                  >
                    {order.status === "Pending" && <Hourglass size={20} />}
                ...
                <p className={`${poppins.className}`}>
                  {" "}
                  Order date: {format(order?.order_date, "dd/MM/yyyy")}
                </p>
                <button
                    ...
                    <table>
                      ...
                      <tbody>
                        {orderedItems?.map((item: any, index) => (
                          <tr key={index} className="text-sm whitespace-nowrap">
